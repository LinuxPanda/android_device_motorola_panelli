    ...
    sp<IGraphicBufferProducer>* p, sp<IGraphicBufferConsumer>* c)
{
    ...
    // Gralloc can not allocate YUV format with odd width or odd height, so we
    // align them to multiple of 2. If pool can support RGB format, we has to
    // check the format before allocate buffer.
    w = ALIGN_CEIL(w, 2);
    h = ALIGN_CEIL(h, 2);
	...
	producer->setMaxDequeuedBufferCount(num);
